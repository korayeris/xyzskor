...
alter table public.matches add column if not exists challenge_week date;
alter table public.matches add column if not exists challenge_league text;
alter table public.matches drop constraint if exists matches_challenge_league_check;
alter table public.matches add constraint matches_challenge_league_check
  check (challenge_league is null or challenge_league in ('super-lig','premier-league','la-liga'));
create index if not exists matches_challenge_week_idx on public.matches(challenge_week, kickoff)
...
begin
  ...
  if p_home not between 0 and 99 or p_away not between 0 and 99 then
...
comment on function public.settle_prediction_challenge_match(text,integer,integer) is
  'SportMonks sonucu geldikten sonra sonucu kilitler; haftadaki 6 seçimin 6sını bilen kullanıcılar için tekil ödül uygunluk kaydı açar.';
