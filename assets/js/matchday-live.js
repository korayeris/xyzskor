(function () {
  ...
  function renderTeamContexts(contexts) {
    ...
    return `<section class="matchday-form-panel"><header><span>TAKIM GÜNDEMİ</span><h3>Son 5 maç ve sıradaki karşılaşma</h3></header><div class="matchday-form-grid">${contexts.map((context) => `<article><h4>${imageTag(context.team_logo,context.team || "Takım","matchday-form-logo")}<span>${esc(context.team || "Takım")}</span></h4><div class="matchday-form-results">${rows(context.recent).length ? rows(context.recent).map((match) => `<div class="is-${esc(String(match.result || "D").toLowerCase())}"><b>${esc(match.result === "W" ? "G" : match.result === "L" ? "M" : "B")}</b>${imageTag(match.opponent_logo,match.opponent,"matchday-form-opponent")}<span>${esc(match.score ? `${match.score.team}-${match.score.opponent}` : "-")}</span><small>${esc(match.opponent)}</small></div>`).join("") : '<p>Son maç verisi yayınlanmadı.</p>'}</div>${context.next ? `<div class="matchday-next"><span>SONRAKİ MAÇ</span>${imageTag(context.next.opponent_logo,context.next.opponent,"matchday-next-logo")}<div><b>${esc(context.next.opponent)}</b><small>${esc(context.next.league || "Futbol")} · ${esc(dateLabel(context.next.kickoff))}</small></div></div>` : '<div class="matchday-next matchday-next--empty">Sonraki maç programı bekleniyor.</div>'}</article>`).join("")}</div></section>`;
  }
