...
function predictionActionHTML(m){
  ...
  return `<div class="prediction-zone"><div class="prediction-label">Maç sonucu</div><div class="predict-form" id="form-${escapeHTML(m.id)}"><div class="pick-row"><button class="pick-btn" type="button" aria-pressed="false" onclick="setPick('${m.id}','1',this)">1 · ${escapeHTML(m.ev)}</button><button class="pick-btn" type="button" aria-pressed="false" onclick="setPick('${m.id}','X',this)">X · Berabere</button><button class="pick-btn" type="button" aria-pressed="false" onclick="setPick('${m.id}','2',this)">2 · ${escapeHTML(m.konuk)}</button></div><div class="mini-note">Kesin skor isteğe bağlıdır.${u?'':' Seçimini kaydetmek için giriş yapman istenir.'}</div><div class="score-row"><span class="mono" style="font-size:12px;color:var(--ink-dim);">Kesin skor</span><input type="number" min="0" max="99" id="sh-${escapeHTML(m.id)}" inputmode="numeric" placeholder="0" aria-label="${escapeHTML(m.ev)} gol sayısı"><span class="mono">—</span><input type="number" min="0" max="99" id="sa-${escapeHTML(m.id)}" inputmode="numeric" placeholder="0" aria-label="${escapeHTML(m.konuk)} gol sayısı"><button class="btn" type="button" id="savePrediction-${escapeHTML(m.id)}" onclick="submitPrediction('${m.id}')">${u?'Kaydet':'Giriş yap ve kaydet'}</button></div><div class="predict-save-status" id="saveStatus-${escapeHTML(m.id)}" role="status" aria-live="polite"></div></div></div>`;
}
...
function renderWeeklyChallenge(){
  ...
  const leagueCounts=['super-lig','premier-league','la-liga'].map(key=>({key,count:matches.filter(match=>match.challengeLeague===key).length}));
  ...
  panel.innerHTML=panel.innerHTML.replace('<h1>Haftalık Challenge</h1><p>Maçları tahmin et, puan topla ve haftalık ligde yüksel.</p>','<h1>6 Maç Challenge</h1><p>2 Süper Lig + 2 Premier League + 2 La Liga maçının sonucunu tahmin et.</p>'+quotaHTML).replace('ÜCRETSİZ · BAHİS YOK','6/6 · 25 BONUS PUAN');
