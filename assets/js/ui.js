...
function footballTeamOfWeekHTML(team){
...
function footballWeeklyCategoriesHTML(players){
  ...
  return `<div class="football-weekly-category-grid">${categories.map(({key,label,player,metric})=>`<article class="football-weekly-category" data-weekly-category="${escapeHTML(key)}"><small>${escapeHTML(label)}</small><div><span>${safeExternalURL(player.playerImage)?`<img src="${escapeHTML(player.playerImage)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'👤'}</span><p><strong>${escapeHTML(player.playerName)}</strong><em>${escapeHTML(player.teamName||'Takım bilgisi bekleniyor')}</em></p><b>${escapeHTML(metric?metric(player):Number(player.score).toFixed(1))}</b></div></article>`).join('')}</div>`;
}
function renderFootballWeeklyFeatures(){
  ...
  const leaders=state.leaders||{},awards=state.awards||{},star=awards.star;
