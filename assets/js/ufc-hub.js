(() => {
  ...
  async function renderRankings() {
    ...
    if (!parsed.length) {
      host.innerHTML = `${head('OFFICIAL BOARD', 'UFC Siralamalari', 'Branslar ve resmî siralar.')}${emptyState('Siralama bulunamadi')}`;
      return;
    }
  ...
  async function renderCompare() {
    ...
    host.innerHTML = `
      ${head('COMPARE', 'Dövüsçü Karsilastirma', `${val(left.name || left.fighterName)} vs ${val(right.name || right.fighterName)}`)}
      <div class="ufcx-grid">
        <article class="ufcx-module">
          ${fighterCard(left)}
          ${fighterCard(right)}
