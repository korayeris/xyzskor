...
declare
  ...
  fake_session uuid;
  result jsonb;
  base_ms bigint := (extract(epoch from now() - interval '10 seconds') * 1000)::bigint;
  valid_events jsonb;
begin
  ...
  select jsonb_agg(jsonb_build_object('type','goal','occurredAt',base_ms + n * 500) order by n)
  ...
  result := public.claim_predict_game_reward(valid_session, test_user, null, 10, 0,
    'GAME_SUCCESS', 'sql-valid-key', 'sql-valid-nonce', valid_events);
  ...
  result := public.claim_predict_game_reward(fake_session, test_user, null, 10, 0,
    'GAME_SUCCESS', 'sql-fake-key', 'sql-fake-nonce', '[]'::jsonb);
