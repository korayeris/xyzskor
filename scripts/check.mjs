...
for (const key of ['SPORTMONKS_API_TOKEN','API_SPORTS_KEY','CITO_API_KEY','OCBLACKTOP_API_KEY','SUPABASE_URL','SUPABASE_ANON_KEY','SUPABASE_SERVICE_ROLE_KEY']) {
  assert.match(rootEnvExample, new RegExp(`^${key}=`, 'm'), `.env.example gerekli ${key} degiskenini belgelemeli.`);
  assert.match(workerEnvExample, new RegExp(`^${key}=`, 'm'), `Worker env ornegi gerekli ${key} degiskenini belgelemeli.`);
}
...
assert.match(documentHtmlRaw, /id="xyzCoreStyleTemplate"[\s\S]*app\.css\?v=314/, 'The heavy core stylesheet must remain route-gated until the football runtime requests it.');
assert.doesNotMatch(documentHtmlRaw, /id="generalHome"|general-home\.js|class="[^"]*general-home/, 'The removed multi-sport card landing must not remain in the root document.');
...
assert.match(footballHubCss, /v317[\s\S]*@media\(forced-colors:none\)[\s\S]*body :where\([^)]*\.wrap[^)]*#multiSportHub[^)]*\.ufcx-shell[^)]*\)[\s\S]*:not\(button\):not\(input\):not\(select\):not\(textarea\)[\s\S]*border-color:transparent\s*!important/i, 'Yapisal hairline temizligi urun yuzeylerini kapsamali, form ve gercek kontrol sinirlarini korumali.');
assert.match(footballHubCss, /\.league-overview-lower\s*>\s*\.league-overview-panel\s*\{\s*box-shadow:none\s*!important[\s\S]*\.league-overview-stories,\.football-weekly-category-grid[\s\S]*background-color:#11171b\s*!important/i, 'Border disinda cizilen lig paneli inset ve bir piksellik grid ayiricilari da kaldirilmali.');
...
assert.doesNotMatch(appBootSource, /\bgeneralHome(?:Path)?\b/, 'The removed static landing fast path must not bypass the football boot.');
assert.match(appBootSource, /var appBootPromise = ensureProductionRuntime\(\)[\s\S]*\.then\(nextTask\)[\s\S]*\.then\(start\)/, 'The canonical root must run the same production football boot as its aliases.');
...
assert.doesNotMatch(dataSource, /supabase-js@2(?:[/'"])/, 'Tarayıcı Supabase SDK kaynağı mutable major etiketi kullanmamalı.');
...
assert.match(appSource, /function filterFootballHomeMatches[\s\S]*activeFootballHomeFilter==='all'\?!row\.classList\.contains\('finished'\)[\s\S]*row\.hidden=hidden;row\.style\.display=hidden\?'none':''[\s\S]*group\.hidden=hidden;group\.style\.display=hidden\?'none':''/, 'Tümü görünümü geçmiş maçları gizlemeli; Geçmiş/canlı/yaklaşan filtreleri computed display durumunu deterministik yönetmeli.');
assert.match(appSource, /function matchHubScopedRows[\s\S]*if\(activeMatchHubFilter==='results'\) return completed;[\s\S]*return \[\.\.\.live,\.\.\.upcoming\];/, 'Tam maç merkezinin Tümü görünümü de geçmiş sonuçları yalnız Sonuçlar filtresine bırakmalı.');
assert.match(appSource, /const featured=liveSpotlight \|\| eligibleFeatured\.find\(match=>footballHomeMatchState\(match\)\.key==='upcoming'\) \|\| null;/, 'Ana vitrinde canlı veya yaklaşan maç yoksa geçmiş karşılaşma öne çıkarılmamalı.');
...
for (const routedClientFile of ['branch-router.js']) {
  assert.match(
...
assert.match(weeklyMigration, /enable row level security[\s\S]*status = 'published'/i, 'Haftalık tablolar RLS ile yalnız published okumaya açılmalı.');
assert.match(memberAdminHardeningMigration, /create or replace function public\.handle_new_user[\s\S]*create trigger on_auth_user_created/i, 'Yeni üyeler için profil üretim fonksiyonu ve auth trigger yeniden kurulmalı.');
assert.match(memberAdminHardeningMigration, /left join public\.profiles[\s\S]*where p\.id is null[\s\S]*on conflict \(id\) do nothing/i, 'Mevcut auth kullanıcılarındaki eksik profiller idempotent backfill edilmeli.');
assert.match(memberAdminHardeningMigration, /revoke all on function public\.list_member_admin_console[\s\S]*from public, anon/i, 'Admin listeleme RPC yüzeyi anon rolüne kapalı olmalı.');
assert.match(memberAdminHardeningMigration, /revoke all on function public\.set_member_admin_role[\s\S]*from public, anon/i, 'Admin yetkilendirme RPC yüzeyi anon rolüne kapalı olmalı.');
assert.match(predictionIntegrityRestoreMigration, /create or replace function public\.enforce_prediction_integrity[\s\S]*kickoff - interval '15 minutes'/i, 'Predict kayıt kilidi veritabanında ve kickoff öncesinde uygulanmalı.');
...
assert.match(dataSource, /const requestedLeague=activeFootballLeague==='all'\?'super-lig':footballLeagueRequestKey\(\)/, 'Challenge yalnızca kullanıcının seçtiği ligi istemeli; Tümü kapsamı güvenli olarak Süper Lig’e dönmeli.');
assert.match(dataSource, /fetchProviderSeasonBundle\(requestedLeague,[\s\S]*\.slice\(0,6\)/, 'Challenge seçili lig için tek sezon isteğinden en fazla altı fikstür seçmeli.');
assert.doesNotMatch(dataSource, /const leagues=\['super-lig','premier-league','la-liga'\][\s\S]*Promise\.all/, 'Challenge açılışta üç lige paralel istek atmamalı.');
...
assert.match(workerSource, /async function resolveFootballHomeBundle[\s\S]*Promise\.allSettled\(FOOTBALL_HOME_LEAGUES\.map/, 'Beş lig paralelliği ve lig bazlı hata sınırı edge katmanında merkezileşmeli.');
...
assert.match(liveFunctionSource('buildFootballPath'), /safeLeague==='all' \? '\/all'[\s\S]*safeLeague==='all' \? '\/' : base/, 'Tüm ligler alt rotaları `/all` kapsamını, futbol ana ürünü ise kanonik `/` kökünü korumalı.');
...
assert.doesNotMatch(workerSource, /jsonResponse\(\{[^\n]*(?:detail|message):[^\n]*(?:providerMessage|safeErrorMessage|error\?\.message)/, 'Public API hata gövdeleri sağlayıcı veya dahili hata metnini yansıtmamalı.');
...
assert.match(documentHtml, /id="clubSocialTitle">Transfer Masası</, 'Kaynak akışı yalnız transfer odaklı ürün adı kullanmalı.');
...
try {
  ...
  globalThis.fetch = async (input) => {
    ...
    if (id) return Response.json({ data:[{ id:`post-${id}`, text:`Resmî transfer ve yeni imza açıklaması ${id}`, created_at:'2026-08-03T08:00:00Z', public_metrics:{like_count:10}, attachments:{media_keys:[`3_media-${id}`]} }], includes:{media:[{media_key:`3_media-${id}`,type:'photo',url:`https://pbs.twimg.com/media/club-${id}.jpg`,width:1600,height:900,alt_text:`Transfer görseli ${id}`}] } });
...
// Coverage yalnızca tanısal/manuel kontrolde uzun ömürlü cache üzerinden okunur.
// Görünür lig geçişi bu yardımcı endpointi otomatik çağırmamalıdır.
...
assert.doesNotMatch(liveFunctionSource('loadFootballLeagueSelection'), /loadFootballCoverage\(/, 'Lig geçişi otomatik coverage isteği başlatmamalı.');
...
// `/` doğrudan futbol ana sayfasıdır; `/futbol/` ve `/all` yalnız takma rotadır.
assert.match(sportBranchesSource, /\["football", "Futbol", "\/"\]/, 'Spor menüsünün ilk öğesi kanonik futbol köküne (`/`) gitmeli.');
assert.doesNotMatch(styleLoaderSource, /\bisGeneralHome\b|general-home-route/, 'Kök rota kaldırılan genel kart yüzeyi için stil istisnası taşımamalı.');
...
await import('./test-membership-game-reward-integrity.mjs');
await import('./test-chat-rls-contract.mjs');
